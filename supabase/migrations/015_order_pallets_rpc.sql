-- RetiFlux™ — Order pallets RPCs (create, pick, complete, release)
-- Migration 015: Run after 014_order_pallets_and_splits.sql

-- Inlined from RUN_ORDER_PALLETS_RPC.sql
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pallets WHERE pallet_id = p_pallet_id) THEN
    RETURN jsonb_build_object('success', false, 'error', 'Order pallet not found: ' || p_pallet_id);
  END IF;

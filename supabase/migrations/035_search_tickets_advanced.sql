-- RetiFlux™ — Advanced ticket search RPC (batch/serial/sku/line/date)
-- Purpose: power dedicated batch search page with DB-enforced visibility.
...
BEGIN
  ...
  IF v_privileged THEN
    ...
    FROM tickets t
    WHERE (t.created_at AT TIME ZONE 'Africa/Nairobi')::date BETWEEN v_start AND v_end
  ...
  -- Non-privileged users: own tickets only, with max 7-day date span.
  ...
  WHERE regexp_replace(upper(coalesce(t.recorded_by, '')), '[^A-Z0-9]', '', 'g') = v_actor_id
    AND (t.created_at AT TIME ZONE 'Africa/Nairobi')::date BETWEEN v_start AND v_end
    AND (v_batch IS NULL OR upper(coalesce(t.batch_lot, '')) LIKE '%' || upper(v_batch) || '%')
    AND (v_serial IS NULL OR upper(coalesce(t.serial, '')) LIKE '%' || upper(v_serial) || '%')
    AND (v_sku IS NULL OR t.sku = v_sku)
    AND (v_line IS NULL OR t.production_line = v_line)
